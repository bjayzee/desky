...
import { createAgencyFolder } from '../utils/storage';
  ...
  try {
    ...
    try {
      ...
      agency = await createAgency(
        { userId, companyName: trimmedCompanyName, ...rest },
        session
      );
      ...
      // Create agency folder in S3 after successful registration
      await createAgencyFolder(agency.companyName);
...
export const verifyResetPassword = async (
  ...
  try {
    ...
    const agency = await updateUserById(foundToken.userId, {
