...
export const getCandidates = () =>
  CandidateModel.find().populate('applications').lean();

export const getCandidateById = (id: string) =>
  CandidateModel.findById(id).populate('applications').lean();

export const getCandidateByEmailWithApplications = (email: string) =>
  CandidateModel.findOne({ email }).populate('applications').lean();

export const getCandidateByEmail = (email: string) =>
  CandidateModel.findOne({ email });

export const createCandidate = (
  values: Partial<ICandidate>,
  session: ClientSession
) =>
  new CandidateModel(values)
    .save({ session })
